...
# Pre-write config BEFORE running the installer.
# The OpenClaw installer's post-install wizard checks for an existing openclaw.json
# and skips the interactive configure step when it finds one.
# Without this, the wizard hangs because there is no TTY.
...
CFGEOF
    # Inline model name substitution (can't use variables inside single-quoted heredoc)
    sed -i "s|REPLACE_MODEL|${DEFAULT_MODEL}|g" "${OPENCLAW_DIR}/openclaw.json"
fi
chmod 600 "${OPENCLAW_DIR}/openclaw.json"
chown agent:agent "${OPENCLAW_DIR}/openclaw.json"
echo "  Config pre-written (wizard will be skipped)"

# Write PayPerQ auth profile now (before installer so it's in place on first boot)
if [ -n "${PAYPERQ_KEY}" ]; then
...
AUTHEOF
    ...
    chown agent:agent "${OPENCLAW_DIR}/agents/main/agent/auth-profiles.json"
    echo "  PayPerQ auth profile written"
else
    echo "  No PayPerQ key — agent will start without LLM credits (top up later)"
fi

# Run installer — with config already on disk, the post-install wizard is skipped
...
# Ensure ownership is correct after install (installer may have touched the dir)
chown -R agent:agent "${OPENCLAW_DIR}"

# Start OpenClaw gateway in foreground mode via nohup.
# Use `gateway run` not `gateway start` — the latter requires systemctl --user
# which is unavailable without a proper PAM login session.
...
[ "$HEALTH_OK" = false ] && echo "  WARNING: Health check failed — gateway may still be booting"
